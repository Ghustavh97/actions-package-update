...
docker build -t actions-package-update --build-arg SET_NODE_VERSION="$SET_NODE_VERSION" --build-arg GITHUB_WORKSPACE="$GITHUB_WORKSPACE" . && docker run actions-package-update -v "env.sh":"/env.sh" -v "entrypoint.sh":"/entrypoint.sh" ${INPUT_ARGS}
